...
for(int i=0;i<A.size();i++){
    ...
    if(tipo.equals("plant based") || tipo.equals("meat and fish") || tipo.equals("diary and eggs")){
     ...
     if(mitt.equals(region)){
     lista=lista+"<h6 class=\"media-heading\"><span class=\"text-bold-500\" "+stiloso+">"+tipo+"<br><small class=\"notification-text\">Import Need</small></span></h6><small class=\"notification-text\" style=\"color:#4E4848;font-weight:500;\">From: "+from+"</small><br><small id = \"trade_request\" class=\"notification-text\" style=\"color:#4E4848;font-weight:500;\">Demand: "+ric.getValore()+"("+ric.getReale()+""+unita+")</small>";
     }else{
     lista=lista+"<h6 class=\"media-heading\"><span class=\"text-bold-500\" "+stiloso+">"+tipo+"<br><small class=\"notification-text\">Export Request</small></span></h6><small class=\"notification-text\" style=\"color:#4E4848;font-weight:500;\">From: "+to+"</small><br><small id = \"trade_request\" class=\"notification-text\" style=\"color:#4E4848;font-weight:500;\">Demand: "+ric.getValore()+"("+ric.getReale()+""+unita+")</small>";	 
     }
     lista=lista+"</div>";
    	 ...
    	 lista=lista+"<div class=\"col-12 px-30 py-75 border-bottom\">";
    	  if(ric.getStatus().equals("Pending")){
    		 stile=" style=\"color:orange;\" ";
    	         }else if(ric.getStatus().equals("Refused")){
     ...
     lista=lista+"</div>";
     lista=lista+"<div class=\"col-12 px-30 py-75 border-bottom\">";
     lista=lista+"<h6 class=\"media-heading mb-0\"><a href=\"javascript:void(0);\" onclick=\"Trade('"+ric.getFrom()+"','"+ric.getTo()+"','"+ric.getTipo()+"','"+ric.getValore()+"','"+ric.getIdp()+"','"+ric.getId()+"','no');Refuse('"+region+"','"+idp+"','"+ric.getId()+"');MyEu('"+region+"','"+idp+"');\">No</a></h6>";
     lista=lista+"</div>";
     lista=lista+"<div class=\"col-12 px-30 py-75\">";
     lista=lista+"<h6 class=\"media-heading mb-0\"><a href=\"#\" onclick=\"Trade('"+ric.getFrom()+"','"+ric.getTo()+"','"+ric.getTipo()+"','"+ric.getValore()+"','"+ric.getIdp()+"','"+ric.getId()+"','test');MyEu('"+region+"','"+idp+"');\">Test it</a></h6>";
     lista=lista+"</div>";
         }
     }
...
if(count>0){
title=title+"<a data-toggle=\"tooltip\" data-placement=\"bottom\" title=\"Notification Center\"><a class=\"nav-link nav-link-label\" href=\"#\" data-toggle=\"dropdown\" style=\"margin-left:40px;\"><b style=\"color:white;\">TRADE </b><b id=\"VREG\" style=\"color:white;\">"+region+"</b>&nbsp;<i class=\"ficon bx bx-bell bx-tada bx-flip-horizontal\" style=\"font-size:24px;\"></i><span class=\"badge badge-pill badge-danger badge-up\">"+count+"</span></a></a>"; 
}else{
title=title+"<a data-toggle=\"tooltip\" data-placement=\"bottom\" title=\"Notification Center\"><a class=\"nav-link nav-link-label\" href=\"#\" data-toggle=\"dropdown\" style=\"margin-left:40px;\"><b style=\"color:white;\">TRADE </b><b id=\"VREG\" style=\"color:white;\">"+region+"</b>&nbsp;<i class=\"ficon bx bx-bell  bx-flip-horizontal\" style=\"font-size:24px;\"></i><span class=\"badge badge-pill badge-danger badge-up\">"+count+"</span></a></a>"; 	
}


title=title+"<ul class=\"dropdown-menu dropdown-menu-media dropdown-menu-right\" style=\"overflow:auto;height:500px;\">";
