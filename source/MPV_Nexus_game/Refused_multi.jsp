...
for(int i=0;i<A.size();i++){
   ...
   if(tipo.equals("plant based") || tipo.equals("meat and fish") || tipo.equals("diary and eggs")){
     ...
     lista=lista+"</div>";
     lista=lista+"<div class=\"col-12 px-30 py-75 border-bottom\">";
     lista=lista+"<h6 class=\"media-heading mb-0\"><a href=\"javascript:void(0);\" onclick=\"Trade('"+ric.getFrom()+"','"+ric.getTo()+"','"+ric.getTipo()+"','"+ric.getValore()+"','"+ric.getIdp()+"','"+ric.getId()+"','no');Refuse('"+region+"','"+idp+"','"+ric.getId()+"');\">No</a></h6>";
     lista=lista+"</div>";
     lista=lista+"<div class=\"col-12 px-30 py-75\">";
     lista=lista+"<h6 class=\"media-heading mb-0\"><a href=\"javascript:void(0);\" onclick=\"Trade('"+ric.getFrom()+"','"+ric.getTo()+"','"+ric.getTipo()+"','"+ric.getValore()+"','"+ric.getIdp()+"','"+ric.getId()+"','test');Test_It('"+region+"','"+idp+"','"+ric.getId()+"');>Test it</a></h6>";
     lista=lista+"</div>";
...
/*
if(count>0){
title=title+"<a class=\"nav-link nav-link-label\" href=\"#\" data-toggle=\"dropdown\" style=\"margin-left:10px;\"><b style=\"color:white;\">TRADE </b><b id=\"VREG\" style=\"color:white;\">"+region+"</b>&nbsp;<i class=\"ficon bx bx-bell bx-tada bx-flip-horizontal\" style=\"font-size:24px;\"></i><span class=\"badge badge-pill badge-danger badge-up\" id=\"counter_notification\">("+count+")</span>  </a>"; 
}else{
title=title+"<a class=\"nav-link nav-link-label\" href=\"#\" data-toggle=\"dropdown\" style=\"margin-left:10px;\"><b style=\"color:white;\">TRADE </b><b id=\"VREG\" style=\"color:white;\">"+region+"</b>&nbsp;<i class=\"ficon bx bx-bell  bx-flip-horizontal\" style=\"font-size:24px;\"></i><span class=\"badge badge-pill badge-danger badge-up\" id=\"counter_notification\">("+count+")</span></a>"; 	
}
*/

title=title+"<ul class=\"dropdown-menu dropdown-menu-media dropdown-menu-right\"  style=\"height:500px;overflow:auto;\">";
title=title+"<li class=\"dropdown-menu-header\">";
title=title+"<div class=\"dropdown-header px-1 py-75 d-flex justify-content-between\"><span class=\"notification-title\">"+count+" new request(s)</span><span class=\"text-bold-400 cursor-pointer\"></span></div>";
title=title+"</li>";
title=title+lista;
title=title+"<li class=\"dropdown-menu-footer\"><a class=\"dropdown-item p-50 text-primary justify-content-center\" href=\"#\" onClick=\"CloseNote();\">CLOSE NOTIFICATIONS</a></li>";
