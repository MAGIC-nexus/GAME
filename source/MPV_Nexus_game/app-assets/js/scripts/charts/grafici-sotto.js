...
var RD_WFgreen_Meat = 2.61;  var RD_WFblue_Meat = 0.1375;  var RD_LF_Meat =  0.0140;
var RD_WFgreen_Diary = 1.24; var RD_WFblue_Diary = 0.1375;  var RD_LF_Diary = 0.0070;
...
//CALCOLO IL VALORE PER LE SOMME DI FOOD CF INTERNAL:
	...
	//------------------------------------------------------------------------------------------------------------------
		...
		if(localStorage.getItem("GameVersion")=="SP1") {
			document.getElementById("CFsp1").innerHTML = "<a data-toggle='tooltip' data-placement='bottom' title='Carbon Footprint of Production' id='cftooltip' style='font-size:1.2rem;color:#E55B3C;'><img src='icon/cb.png' style='width:24px;'></a>";
			if(titoloCF_sotto >16) {
				document.getElementById("CFsp1Value").innerHTML = '<a data-toggle="tooltip" data-placement="bottom" title="Target value &le;16" id="" style="font-size:1.2rem;color:#E55B3C;"><font color="red" style="font-size:1.2rem">'+titoloCF_sotto+'</font></a>';
			}
			else {
				document.getElementById("CFsp1Value").innerHTML = '<a data-toggle="tooltip" data-placement="bottom" title="Target value &le;16" id="" style="font-size:1.2rem;color:#E55B3C;"><font color="#B1FB17" style="font-size:1.2rem">'+titoloCF_sotto+'</font></a>';
			}
		}
		*/
    ...
    /*	    	var prima = Math.round(Number(dataCF_food_int)+Number(datoCFen_internal));
    	 var grand_total_ante = (prima-initial_grafici_sotto_cf)/initial_grafici_sotto_cf;
			//console.log("WFBLUE PRIMA DEL TITOLO, CALCOLO TITOLO PERCENTUALE "+grand_total_ante);
    	    	...
    	    	//----PERCENTUALE SOTTO AL TITOLO---
    	    	var rounded = Math.round(parseFloat(initial_grafici_sotto_cf).toFixed(2));
    	    	...
    	    	var tit = Math.round(titoloCF_sotto);
    	    	var percent = tit-rounded;
    	    	if(-1<percent && percent<1)
    	    		percent=0.0;
    	    	
				if(percent>=1) {
					document.getElementById("percent1_sotto").innerHTML = "<font color='red'>"+grand_total+ "% above initial value</font>";
				}
				else if(percent<=-1) {
					document.getElementById("percent1_sotto").innerHTML = "<font color='green'>"+grand_total+ "% below initial value</font>";
				} 
				else {
					document.getElementById("percent1_sotto").innerHTML = "<font color='grey'>Equals initial value</font>";
				}
...
if((checkregion != null)&&(checkregion.getAttribute('id')=='regionA')) {
	...
	var rounded = Math.round(parseFloat(initial_grafici_sotto_lf).toFixed(2));
	var tit = Math.round(titoloLF_sotto);
	...
	if(percent>=1) {
		document.getElementById("percent2_sotto").innerHTML = "<font color='red'>"+grand_total+ "% above initial value</font>";
	}
	else if(percent<=-1) {
		document.getElementById("percent2_sotto").innerHTML = "<font color='green'>"+grand_total+ "% below initial value</font>";
	} 
	else {
		document.getElementById("percent2_sotto").innerHTML = "<font color='grey'>Equals initial value</font>";
	}
	...
	var datiaggbar0 = [sumLFfooden];
		...
		horizontalChart2.options.scales.yAxes[0].ticks.max = maxYLF;
		horizontalChart2.options.annotation.annotations[0].value = physicalLF;
		horizontalChart2.options.annotation.annotations[1].value = sustainableLF;
...
//  		//console.log("MULCHING PARSE: "+Mulching_parse);
	...
	*/
	...
	var rounded = Math.round(parseFloat(initial_grafici_sotto_wfg).toFixed(2));
	var tit = Math.round(titoloWFg_sotto);
	...
	if(percent>=1) {
		document.getElementById("percent3_sotto").innerHTML = "<font color='red'>"+grand_total+ "% above initial value</font>";
	}
	else if(percent<=-1) {
		document.getElementById("percent3_sotto").innerHTML = "<font color='green'>"+grand_total+ "% below initial value</font>";
	} 
	else {
		document.getElementById("percent3_sotto").innerHTML = "<font color='grey'>Equals initial value</font>";
	}  		  /////////////////////////////////////////////////////////////////
  ...
  var sumWFgfooden = Math.round(parseInt(Number(dataWFgreen_food_int)+Number(datoWFgreen_en_internal)));
	...
	var datiaggbar0 = [sumWFgfooden];
		...
		horizontalChart3.options.scales.yAxes[0].ticks.max = maxYWFG;
...
var physicalWFB = 0;
  	...
  	//fine button clicks
		...
		//console.log("diaryA: "+diaryA+" diaryB: "+diaryB+" diaryC: "+diaryC+" diary_WFblue: "+diary_WFblue);
  		...
  		//console.log("WFBlue En Internal: "+datoWFblue_en_internal);
  		//-----------------------------------------------------------------------------------------------
  		/*
    	...
    	var rounded = Math.round(parseFloat(initial_grafici_sotto_wfb).toFixed(2));
		var tit = Math.round(titoloWFb_sotto);
		...
		if(percent>=1) {
			...
			document.getElementById("percent4_sotto").innerHTML = "<font color='grey'>Equals initial value</font>";
		}
		...
		horizontalChart4.options.scales.yAxes[0].ticks.max = maxYWFB;
