...
function Sustanaible_Limit() {
	...
	if(Version == "SP1") {
		...
		if(enablePopup==1) {
			setTimeout(
				function(){ 
					alert("Good job! You managed to achieve the targets for carbon, water and land footprint reduction. Now you can try to: > replay Single Player Level 1 for one of the other EU regions and try to achieve the objective with a more desirable set of choices that have less impact on society (lower ‘desirability penalty’). > step it up a notch with Single Player Level 2, taking control over all four EU regions."); 
				}, 
			1000);
			enablePopup++;
		}
