...
//START CALCOLO ELECTRIFICATION TRANSPORT REGION A
...
//-----RISULTATO: --------
var elec_tr = (Number(el_tr)/Number(fossil_tr))*100;  //elec '2'
//console.log("REGION A ELECTRIFICATION TRANSPORT VALORE: "+elec_tr);
////////////////
var gas_coal = '0';   //gas
...
vfossil_f = Number(vfossil_f)/100;
var vbiofuel = document.getElementById("L31").value;
vbiofuel = Number(vbiofuel)/100;
var velectricity = document.getElementById("L32").value;
velectricity = Number(velectricity)/100;
///INIZIO CALCOLO PERCENTAGE FUEL (SOSTITUIRE POI CON OGGETTO LOCAL STORAGE)

//----------------------------------------------------------------------------------------------------
var part_gas_1 = (bio_ind+bio_tr+bio_s+bio_house);
var part_gas_2 = (el_ind+el_tr+el_s+el_house); 
var perc_fuel = part_gas_1/((part_gas_1+part_gas_2)*100); //perc_fossil '26'
console.log ("TOTAL BIO USER: "+total_bio_user);
console.log("TOTAL_EL_USER: "+total_el_user);
console.log("PERCENTUALE BIOFUEL PENALITY: "+perc_fuel);
...
if(vred_tr>red_tr) {
	...
	if(vred_tr>treshold_reduction_en_tr) {
		...
		penality_reduction_en_tr = Number(penality_reduction_en_tr);
...
if(vred_service>red_service) {
	...
	if(vred_service>treshold_reduction_en_service) {
		...
		penality_reduction_en_service = Number(penality_reduction_en_service);
...
var penality_reduction_en_service = 0;
	...
	if(vred_house>red_house) {
		if(vred_house>treshold_reduction_en_house) {
			...
			penality_reduction_en_house =Number(penality_reduction_en_house);
	}		else {
...
var penality_reduction_en_service = 0;
	...
	if(velec_tr>elec_tr) {
		if(velec_tr>treshold_elec_tr) {
			var calc1 = (Number(-elec_tr) + Number(treshold_elec_tr)) * penality1_elec_tr;
			//console.log("REGION D ELETRICITY calc1"+calc1);

			var calc2 = (Number(-treshold_elec_tr) + Number(velec_tr)) * penality2_elec_tr;
			//console.log("REGION D ELETRICITY calc2"+calc2);
			var penality_elec_tr = (calc1)+(calc2);
			//console.log("REGION D ELETRICITY pen"+penality_elec_tr);

			penality_elec_tr = parseFloat(penality_elec_tr).toFixed(4);
			//console.log("REGION D ELETRICITY pen parse"+penality_elec_tr);

			penality_elec_tr = Number(penality_elec_tr);
			//console.log("REGION D ELETRICITY pen parse"+penality_elec_tr);
