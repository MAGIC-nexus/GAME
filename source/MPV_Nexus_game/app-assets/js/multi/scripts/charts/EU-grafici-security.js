...
EU_security_food_rate=92;
EU_security_energy_rate=52;
...
var blu = $primary;
  ...
  //Doughnut Chart SECURITY
    ...
    var doughnutChartctx2 = $("#EU-polar-chart-security");
    ...
    var doughnutchartData2 = {
      labels: ["Region A", "Region B", "Region C", "Region D", "WORLD"],
      datasets: [{
        label: "Production of Food",
        data: [3348,0,0,0,203425],
    ...
    Chart.pluginService.register({
    	  beforeDraw: function(chart) {
    	    ...
    	    var text = EU_security_food_rate,
...
function EU_security_food() {
	...
	D = D_int+D_ext;
	//////console.log("D_security_food: "+D_security_energy+" D_A_ext: "+D_A_ext+" D_B_ext: "+D_B_ext+" D_C_ext: "+D_C_ext);
	...
	*/
	...
	//document.getElementById("eufood").innerHTML = Math.round(EU_security_food_rate);
	
	var EU = JSON.parse(localStorage.getItem("EU"));
	EU["food"] = EU_security_food_rate;
	localStorage.setItem("EU", JSON.stringify(EU));
	
	if(EU_security_food_rate>=95) {
		document.getElementById("eufood").innerHTML="<font color='green'>"+EU_security_food_rate+"%</font>";
	}
	else {
		document.getElementById("eufood").innerHTML="<font color='red'>"+EU_security_food_rate+"%</font>";
	}

	var datiagghor = [EU_A_security_food, EU_B_security_food, EU_C_security_food, EU_D_security_food, WORLD_security_food];
  	doughnutSimpleChart2.data.datasets[0].data = datiagghor;
  	doughnutSimpleChart2.update();
}	
setInterval(EU_security_food, 3000);

// Doughnut Chart
    // ---------------------------------------------
    //Get the context of the Chart canvas element we want to select
    var doughnutChartctx3 = $("#EU-polar-chart-security-2");
    ...
    Chart.pluginService.register({
  	  beforeDraw: function(chart) {
  	    ...
  	    var text = EU_security_energy_rate,
  	        textX = Math.round((width - ctx.measureText(text).width) / 2),
  	      textY = height / 1.6 ;
...
function EU_security_energy() {
	...
	//document.getElementById("euenergy").innerHTML = Math.round(EU_security_energy_rate);

	var EU = JSON.parse(localStorage.getItem("EU"));
	EU["energy"] = EU_security_energy_rate;
	localStorage.setItem("EU", JSON.stringify(EU));
	
	if(EU_security_energy_rate>=70) {
		document.getElementById("euenergy").innerHTML="<font color='green'>"+EU_security_energy_rate+"%</font>";
	}
	else {
		document.getElementById("euenergy").innerHTML="<font color='red'>"+EU_security_energy_rate+"%</font>";
	}

	var datiagghor = [EU_A_security_energy, EU_B_security_energy, EU_C_security_energy, EU_D_security_energy, WORLD_security_energy];
  	doughnutSimpleChart3.data.datasets[0].data = datiagghor;
  	doughnutSimpleChart3.update();
}
setInterval(EU_security_energy, 3000);
